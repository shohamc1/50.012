...
class GoBackN:
    ...
    NO_PREV_ACK_MSG = (
        "Don't have previous ACK to send, will wait for server to timeout."
    )
    ...
    def handle_arrival_msg(self):
        ...
        if msg_data.is_corrupt:
            if self.is_receiver:
                ...
                util.log(
                    "Received corrupted data. Resending ACK: "
        ...
        # If DATA message, assume its for receiver
            ...
            if msg_data.seq_num == self.expected_sequence_number:
                ...
                if self.expected_sequence_number == 0:
                    ...
                    return
                util.log(
                    "DATA message had unexpected sequence #"
    ...
    def _timeout(self):
        util.log(
            "Timeout! Resending all packets in window. Resending packets with seq #s "
            + str(self.sender_base)
            + "-"
            + str(self.next_sequence_number - 1)
            + "."
        )
        self.sender_lock.acquire()
        if self.timer.is_alive():
