...
class StopAndWait:
    ...
    def handle_arrival_msg(self):
        ...
        if msg_data.is_corrupt:
            if self.is_receiver:
                if not self.last_pkt_sent_data:
                    return
                util.log(
                    "Received corrupt data. Resending: "
        ...
        if msg_data.msg_type == config.MSG_TYPE_ACK:
            if (
                self.sender_state == config.WAIT_FOR_ACK_MSG
                and msg_data.seq_num == self.sequence_number
            ):
                util.log(
                    "Received ACK with expected seq #. " + util.pkt_to_string(msg_data)
                )
                self.sender_lock.acquire()
                self.timer.cancel()
                self.sequence_number = not (
        ...
        # If DATA message, assume its for receiver
            ...
            if msg_data.seq_num == self.sequence_number:
                ...
                ack_pkt = util.make_packet(
                    b"", config.MSG_TYPE_ACK, self.sequence_number
                )
